...
1. [Адаптивный AI-агент для определения профессии](#адаптивный-ai-агент-для-определения-профессии)  
   1.1. [Планирование](#планирование) — [Проблема](#проблема) · [Цель](#цель) · [Решение](#решение)  
   1.2. [Команда (о нас)](#команда-о-нас)  
   1.3. [Конкурентный SWOT-анализ](#конкурентный-swot-анализ) — [Ключевые находки из научной литературы](#ключевые-находки-из-научной-литературы-коротко)  
   1.4. [Таблица требований](#таблица-требований)  
   1.5. [Моделирование процессов и функций AI-ассистента](#моделирование-процессов-и-функций-ai-ассистента) — [Акторы](#акторы) · [UML-диаграмма развертывания](#uml-диаграмма-развертывания-deployment-diagram) · [UML-диаграмма компонентов](#uml-диаграмма-компонентов-component-diagram)  
   1.6. [Проектирование базы данных](#проектирование-базы-данных) — [Users](#1-users-пользователи) · [UserProfiles](#2-userprofiles-дополнительные-данные-о-пользователе) · [Tests](#3-tests-типы-тестов) · [TestResults](#4-testresults-результаты-тестов) · [CareerSuggestions](#5-careersuggestions-рекомендации-по-профессиям) · [AnalyticsLogs](#6-analyticslogs-логи-аналитики--экспорта)  
   1.7. [Работа с формой](#работа-с-формой)  
   1.8. [Описание методик](#описание-методик) — [Тест MBTI](#1-тест-mbti-myersbriggs-type-indicator) · [Якоря карьеры](#2-якоря-карьеры-career-anchors-климова) · [Тип профессии RIASEC/«обпрез»](#3-тип-профессии-типы-riasec--обпрез) · [Тест Климова (DDO)](#4-тест-климова-ddo) · [Тест Голланда (RIASEC)](#5-тест-голланда-riasec) · [Тесты по сегментам](#тесты-структурированные-по-сегментам)  
   1.9. [Как построить адаптивный диалог](#как-построить-адаптивный-диалог) — [Как GPT-ассистент может обрабатывать методики](#как-gpt-ассистент-может-обрабатывать-эти-методики) · [Что лучше и почему](#что-лучше-и-почему) · [Шаги обработки GPT](#как-реализовать-обработку-gpt-шаги) · [Пример результата](#пример-результата-обработки)
